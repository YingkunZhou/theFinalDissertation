...
class DatToCtlIo(implicit val conf: CpuConfig) extends Bundle()
{
   val dec_inst    = Output(UInt(conf.instWidth.W))
...
class DpathIo(implicit val conf: CpuConfig) extends Bundle()
{
...
class DatPath(implicit val conf: CpuConfig) extends Module
{
   ...
   val dec_reg_pc            = RegInit(0.asUInt(conf.addrWidth.W))

   // Execute State
   val exe_reg_inst          = RegInit(BUBBLE)
   val exe_reg_pc            = RegInit(0.asUInt(conf.addrWidth.W))
   val exe_reg_wbaddr        = Reg(UInt(5.W))
   val exe_reg_rs1_addr      = Reg(UInt(5.W))
   val exe_reg_rs2_addr      = Reg(UInt(5.W))
   val exe_reg_op1_data      = Reg(UInt(conf.dataWidth.W))
   val exe_reg_op2_data      = Reg(UInt(conf.dataWidth.W))
   val exe_reg_rs2_data      = Reg(UInt(conf.dataWidth.W))
   ...
   val mem_reg_inst          = Reg(UInt(conf.instWidth.W))
   val mem_reg_alu_out       = Reg(Bits())
   val mem_reg_wbaddr        = Reg(UInt())
   val mem_reg_rs1_addr      = Reg(UInt())
   val mem_reg_rs2_addr      = Reg(UInt())
   val mem_reg_op1_data      = Reg(UInt(conf.dataWidth.W))
   val mem_reg_op2_data      = Reg(UInt(conf.dataWidth.W))
   val mem_reg_rs2_data      = Reg(UInt(conf.dataWidth.W))
   ...
   val wb_reg_wbdata         = Reg(UInt(conf.dataWidth.W))
   ...
   val if_pc_plus4 = (if_reg_pc + 4.asUInt(conf.addrWidth.W))
   ...
   // Bypass Muxes
   val exe_alu_out  = Wire(UInt(conf.dataWidth.W))
   val mem_wbdata   = Wire(UInt(conf.dataWidth.W))

   val dec_op1_data = Wire(UInt(conf.dataWidth.W))
   val dec_op2_data = Wire(UInt(conf.dataWidth.W))
   val dec_rs2_data = Wire(UInt(conf.dataWidth.W))
   ...
   val exe_adder_out = (exe_alu_op1 + exe_alu_op2)(conf.dataWidth-1,0)
   ...
   exe_alu_out := MuxCase(exe_reg_inst.asUInt(), Array(
                  ...
                  (exe_reg_ctrl_alu_fun === ALU_SLL)  -> (exe_alu_op1 << alu_shamt)(conf.dataWidth-1, 0).asUInt(),
