...
class MemPortIo(data_width: Int)(implicit val conf: CpuConfig) extends Bundle
{
...
class MemReq(data_width: Int)(implicit val conf: CpuConfig) extends Bundle
{
...
class AsyncScratchPadMemory(num_core_ports: Int, num_bytes: Int = (1 << 21), data_Width: Int)(implicit val conf: CpuConfig) extends Module
{
   val io = IO(new Bundle
   {
      val core_ports = Vec(num_core_ports, Flipped(new MemPortIo(data_width = data_Width)) )
      val debug_port = Flipped(new MemPortIo(data_width = 32))
   })
   val num_bytes_per_line = 8
   val num_lines = num_bytes / num_bytes_per_line
   println("\n    Tile: creating Asynchronous Scratchpad Memory of size " + num_lines*num_bytes_per_line/1024 + " kB\n")
...
class SyncScratchPadMemory(num_core_ports: Int, num_bytes: Int = (1 << 21))(implicit val conf: CpuConfig) extends Module
{
   val io = IO(new Bundle
   {
   ...
   val num_bytes_per_line = 8
   val num_lines = num_bytes / num_bytes_per_line
   println("\n    Tile: creating Synchronous Scratchpad Memory of size " + num_lines*num_bytes_per_line/1024 + " kB\n")
...
class ScratchPadMemory(num_core_ports: Int, num_bytes: Int = (1 << 21), seq_read: Boolean = false)(implicit val conf: CpuConfig) extends Module
{
   val io = IO(new Bundle
   {
      val core_ports = Vec(num_core_ports, Flipped(new MemPortIo(data_width = conf.xprlen)) )
...
object LoadDataGen
{
   def apply(data: Vec[UInt], typ: UInt, idx: UInt) : UInt =
   {
      ...
      Cat(dout_31_16,Cat(dout_15_8,dout_7_0))
   }
}
