...
import chisel3._
import chisel3.util.{DecoupledIO, Mux1H}
import Util._

object DMConsts{

  def dmiDataSize = 64 // FIXME: change to 64?
  def nDMIAddrSize = 7
  def dmiOpSize = 2
...
/** Structure to define the top-level DMI interface
  ...
  */
class DMIIO(implicit val conf: CpuConfig) extends Bundle {
...
class SimDTM(implicit val conf: CpuConfig) extends BlackBox {
  val io = IO(new Bundle {
      val clk = Input(Clock())
      val reset = Input(Bool())
      val debug = new DMIIO()
      val exit = Output(UInt(conf.xprlen.W))
    })
...
class DebugDPath(implicit val conf: CpuConfig) extends Bundle
{
  // REG access
  val addr = Output(UInt(5.W))
  val wdata = Output(UInt(conf.xprlen.W))
  val validreq = Output(Bool())
  val rdata = Input(UInt(conf.xprlen.W))
  val resetpc = Output(Bool())
}

class DebugCPath(implicit val conf: CpuConfig) extends Bundle
{
  val halt = Output(Bool())
}

class DebugIo(implicit val conf: CpuConfig) extends Bundle
{
  val dmi = Flipped(new DMIIO())
  val ddpath = new DebugDPath()
  val dcpath = new DebugCPath()
  val debugmem = new MemPortIo(data_width = conf.xprlen) // FIXME: change to 64?
  val resetcore = Output(Bool())
}

class DebugModule(implicit val conf: CpuConfig) extends Module {
  ...
  sbcsreset.sbasize := 64.U       // FIXME: change to 64?
  sbcsreset.sbaccess64 := true.B
  sbcsreset.sbaccess32 := false.B
