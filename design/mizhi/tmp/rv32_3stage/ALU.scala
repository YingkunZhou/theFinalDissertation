...
class ALUIO(implicit val conf: CpuConfig) extends Bundle {
  val fn = Input(UInt(SZ_ALU_FN.W))
  val in2 = Input(UInt(conf.dataWidth.W))
  val in1 = Input(UInt(conf.dataWidth.W))
  val out = Output(UInt(conf.dataWidth.W))
  val adder_out = Output(UInt(conf.dataWidth.W))
}

class ALU(implicit val conf: CpuConfig) extends Module
{
  val io = IO(new ALUIO)

  val msb = conf.dataWidth-1
