...
class MemArbiter(implicit val conf: CpuConfig) extends Module
{
   val io = IO(new Bundle
      {
         // TODO I need to come up with better names... this is too confusing
         // from the point of view of the other modules
         val imem = Flipped(new MemPortIo(conf.instWidth)) // instruction fetch
         val dmem = Flipped(new MemPortIo(conf.dataWidth)) // load/store
         val mem  = new MemPortIo(conf.dataWidth)      // the single-ported memory
      })

   //***************************
   val i1reg = Reg(UInt(conf.instWidth.W))
   val d1reg = Reg(UInt(conf.dataWidth.W))
