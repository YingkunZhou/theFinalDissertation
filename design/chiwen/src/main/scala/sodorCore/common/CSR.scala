...
class PerfCounterIO(implicit val conf: Configure) extends Bundle{
  //val eventSel = Output(UInt(conf.datalen.W))
  val inc = Input(UInt(conf.datalen.W))
...
class CSRFileIO(implicit val conf: Configure) extends Bundle {
  val hartid = Input(UInt(conf.datalen.W))
  val rw = new Bundle {
    val cmd   = Input(UInt(CSR.SZ))
    val rdata = Output(UInt(conf.datalen.W))
    val wdata = Input(UInt(conf.datalen.W))
  }
  ...
  val evec      = Output(UInt(conf.datalen.W))
  val exception = Input(Bool())
  val retire    = Input(Bool())
  val pc        = Input(UInt(conf.datalen.W))
  val time      = Output(UInt(conf.datalen.W))
  val counters  = Vec(60, new PerfCounterIO)

}

class CSRFile(implicit val conf: Configure) extends Module
{
  ...
  val reg_mepc     = Reg(UInt(conf.datalen.W))
  val reg_mcause   = Reg(UInt(conf.datalen.W))
  val reg_mtval    = Reg(UInt(conf.datalen.W))
  val reg_mscratch = Reg(UInt(conf.datalen.W))
  val reg_mtimecmp = Reg(UInt(conf.datalen.W))
  val reg_medeleg  = Reg(UInt(conf.datalen.W))

  val reg_mip = RegInit(0.U.asTypeOf(new MIP()))
  val reg_mie = RegInit(0.U.asTypeOf(new MIP()))
  val reg_wfi = RegInit(false.B)
  val reg_mtvec = Reg(UInt(conf.datalen.W))
  ...
  val reg_dpc = Reg(UInt(conf.datalen.W))
  val reg_dscratch = Reg(UInt(conf.datalen.W))
