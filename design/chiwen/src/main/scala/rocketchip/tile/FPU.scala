...
import rocketchip.rocket.Instructions._
...
class FPUDecoder(implicit p: Parameters) extends FPUModule()(p) {
  ...
  val f =
    Array(
      FLW      -> List(Y,Y,N,N,N,X,X,X,X,N,N,N,N,N,N,N),
  ...
  val d =
    Array(
      FLD      -> List(Y,Y,N,N,N,X,X,X,N,N,N,N,N,N,N,N),
...
case class FType(exp: Int, sig: Int) {
  ...
  def unsafeConvert(x: UInt, to: FType) = if (this == to) x else {
    ...
    val expOut = {
      ...
      val commonCase = (expIn + (1 << to.exp).asUInt) - (1 << exp).asUInt
...
class FPU(cfg: FPUParams)(implicit p: Parameters) extends FPUModule()(p) {
  ...
  @chiselName class FPUImpl { // entering gated-clock domain
    ...
    def useScoreboard(f: ((Pipe, Int)) => Bool): Bool = pipes.zipWithIndex.filter(_._1.lat > 3).map(x => f(x)).fold(Bool(false))(_||_)
